...
class EvolutionAction(str, Enum):
    """Actions that can be taken during progressive evolution."""
...
class FeatureDefinition(BaseModel):
    ...
    llm_chain_of_thought_reasoning: Optional[str] = Field(
        None,
        description="Stored chain-of-thought reasoning from the LLM during feature generation",
    )
...
class FeatureSet(BaseModel):
    """Collection of features at a specific stage of evolution."""

    features: List[FeatureDefinition] = Field(..., description="List of features")
    action_taken: EvolutionAction = Field(
        default=EvolutionAction.GENERATE_NEW, description="Action that created this set"
    )
...
class DataContext(BaseModel):
    ...
    # Data splits
    train_data: Optional[Union[pd.DataFrame, Dict[str, Any]]] = Field(
        ..., description="Training data"
    )
    validation_data: Optional[Union[pd.DataFrame, Dict[str, Any]]] = Field(
        ..., description="Validation data"
    )
    test_data: Optional[Union[pd.DataFrame, Dict[str, Any]]] = Field(
    ...
    def get_column_info(self, column: str) -> Optional[Dict[str, Any]]:
        ...
        if self.train_data is not None and column in self.train_data:
...
class ActionContext(BaseModel):
    """Context for deciding next evolution action."""
    ...
    available_actions: List[EvolutionAction] = Field(
    ...
    def can_add_feature(self) -> bool:
        ...
        return len(self.current_features.features) < self.max_features
